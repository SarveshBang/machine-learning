...
B) timesfm_chronos_venv  (for `timesfm_model`, `chronos_model`)
   - Python 3.10.18
   - numpy==1.26.4, pandas==2.1.4, scikit-learn==1.6.1, torch==2.5.1
...
C) uni2ts_venv  (for `uni2ts_model`)
   - Python 3.10.18
   - numpy==1.26.0, pandas==2.1.4, scikit-learn==1.7.0, torch (as installed with your CUDA/CPU)
   - gluonts==0.14.4, uni2ts==1.2.0
...
Key Parameters to Keep Consistent
---------------------------------
- `WINDOW` (rolling context length) – set default to 21 trading days (best performing window) in each notebook. (Could be changed to test/validate other window sizes)
...
Exact Artifacts Produced (Results/)
-----------------------------------
...
---------------------------
Core libraries/models: scikit‑learn, XGBoost, PyTorch, TimesFM (Google), Chronos (AWS), Uni2TS/Moirai (Salesforce/Meta & partners), GluonTS.
Empirical evaluation follows conventions inspired by:
- “Empirical Asset Pricing via Machine Learning” (Gu, Kelly, Xiu).
- “Deep Learning in Asset Pricing” (Gu, Kelly, Xiu).

License
-------
Code is provided for academic reproducibility. Data files are **not** included.
